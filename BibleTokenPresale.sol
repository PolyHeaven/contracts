...
contract BibleTokenPresale is ReentrancyGuard {
    ...
    function getBIBLETokensLeft() external view returns (uint256) {
        return BIBLE.balanceOf(address(this));
    }

    function claimTokens () external {
        require (isClaimActive, "Claim is not allowed yet");
        require (tokensOwned[msg.sender] > 0, "User should own some BIBLE tokens");
    ...
    function withdrawUnsoldBIBLE() external onlyOwner {
        uint256 amount = BIBLE.balanceOf(address(this)) - totalTokensSold;
        BIBLE.safeTransfer(msg.sender, amount);
    }

    function withdrawAllBIBLE() external onlyOwner {
        BIBLE.safeTransfer(msg.sender, BIBLE.balanceOf(address(this)));
    }
}
